...
> **Formerly Awesome Agent Skills.** Agent Skills 仍然是本项目的一等公民；现在，我们正在把目录扩展到更完整的 Agent Plugin 生态。
...
> 如果觉得这个项目对你有所帮助，还请帮忙点个 🌟 让更多人知晓。同时，也欢迎关注我的 𝕏 账号 [@李不凯正在研究](https://x.com/libukai)，获取有关 Agent Plugin、Agent Skill 和 MCP 的最新资源与实战教程！

## 从 Skill 到 Plugin

Plugin 是更接近最终用户的安装与分发单元。一个 Plugin 可以只包含一个 Skill，也可以把任务说明、工具连接、专用 Agent、自动化 Hook 和交互界面组合成一个完整能力包。
