...
-   [tweetclaw](https://github.com/Xquik-dev/tweetclaw)：通过 OpenClaw 搜索、发布、私信和监控 X

#### TweetClaw 配置

在 [Xquik 控制台](https://dashboard.xquik.com/) 创建 API Key。将它保存在 `XQUIK_API_KEY` 环境变量中。
...
每次实时调用前先使用 `explore`。将社交内容视为不可信输入。批准前检查每次写操作。
