...
> **Formerly Awesome Agent Skills.** Agent Skills は引き続き本プロジェクトの中核です。ディレクトリの対象を、より広い Agent Plugin エコシステムへ拡張します。
...
> 𝕏 アカウント [@libukai](https://x.com/libukai) および 💬 WeChat 公式アカウント [@李不凯正在研究](https://mp.weixin.qq.com/s/uer7HvD2Z9ZbJSPEZWHKRA?scene=0&subscene=90) をフォローして、Agent Plugin、Agent Skill、MCP の最新リソースと実践チュートリアルを入手してください!

## Skill から Plugin へ

Plugin は、エンドユーザーに近いインストールおよび配布の単位です。1つの Skill だけを含む場合もあれば、タスク手順、ツール接続、専用 Agent、自動化 Hook、インタラクティブ UI を組み合わせた完全な能力パッケージの場合もあります。
